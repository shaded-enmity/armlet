...
function wobblePath(pts, rng, amp, closed = false) {
  if (amp === 0) {
...
function rectPath(x, y, w, h, rng, amp) {
  return wobblePath(
...
function defs(spec) {
  ...
  if (spec.filter === "whimsical-shadow") {
    ...
    const ct = n("feComponentTransfer", { in: "off" });
    ct.appendChild(n("feFuncA", { type: "linear", slope: "0.35" }));
    f.appendChild(ct);
...
function validate(spec) {
  ...
  if (typeof spec.name !== "string") throw new Error("'name' must be string");
  if (typeof spec.total !== "number" || !Number.isInteger(spec.total) || spec.total <= 0)
    throw new Error("'total' must be positive integer");
  if (!Array.isArray(spec.members)) throw new Error("'members' must be array");
}

export function renderBitlayoutSVG(spec, container, options = {}) {
  validate(spec);
  ...
  baseG.appendChild(
    n("path", {
      d: wobblePath([[padX, valueY], [padX + innerW, valueY]], rng, ss.wobbleAmp),
      fill: "none",
      stroke: ss.stroke,
      "stroke-width": ss.strokeWidth,
      "stroke-linecap": "round",
  ...
  spec.members.forEach((m, i) => {
    ...
    if (i < spec.members.length - 1) {
      baseG.appendChild(
        n("path", {
          d: wobblePath([[x + w, indexY], [x + w, valueY + valueH]], rng, ss.wobbleAmp),
          fill: "none",
          stroke: ss.stroke,
          "stroke-width": ss.strokeWidth,
          "stroke-linecap": "round",
